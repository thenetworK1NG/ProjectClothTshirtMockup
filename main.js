...
function drawUVEditor() {
    ...
    // Draw all images with proper scaling
    images.forEach((imgData, index) => {
        ...
        uvCtx.scale(imageScale, imageScale);
...
function updateTexture() {
    ...
    // Only draw images if there are any
    if (images.length > 0) {
    // Draw all images
    ...
    tshirtModel.traverse((child) => {
        if (child.isMesh) {
            // Dispose of the old material and texture if they exist
            if (child.material) {
                ...
                <div class="light-controls">
                    <div class="light-group">
                        ...
                        <div class="light-input">
                            ...
                            <input type="range" id="front-light-intensity" min="0" max="2" value="1" step="0.1">
                            <span id="front-light-value">1.0</span>
                        </div>
                        <div class="light-input">
                            <label>Color</label>
                    ...
                    <div class="light-group">
                        ...
                        <div class="light-input">
                            <label>Intensity</label>
                            <input type="range" id="back-light-intensity" min="0" max="2" value="0.8" step="0.1">
                            <span id="back-light-value">0.8</span>
                        </div>
                        <div class="light-input">
                            <label>Color</label>
...
// Add styles for the shirt panel
...
// Update the CSS for UI hiding
renderStyles.textContent = renderStyles.textContent.replace(
    /body\.ui-hidden.*?{/s,
    `body.ui-hidden {
        margin: 0 !important;
        padding: 0 !important;
    ...
    body.ui-hidden > *:not(#canvas-container) {
        display: none !important;
    }
    ...
    /* Hide scrollbars when UI is hidden */
    body.ui-hidden::-webkit-scrollbar {
        display: none !important;
    }`
);
...
function setupPanelMinimize(panel) {
    ...
    newMinimizeBtn.addEventListener('click', (e) => {
        ...
        const panelType = panel.classList.contains('scene-panel') ? 'scene' : 
                         panel.classList.contains('camera-info-panel') ? 'camera' : 'render';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setupPanelMinimize(cameraInfoPanel);
...
function animate() {
    ...
    document.getElementById('cam-target-x').textContent = controls.target.x.toFixed(2);
    document.getElementById('cam-target-y').textContent = controls.target.y.toFixed(2);
    document.getElementById('cam-target-z').textContent = controls.target.z.toFixed(2);
    ...
    if (!isReplayingCamera) {
        ...
        const moveY = (keys.ArrowUp ? 1 : 0) - (keys.ArrowDown ? 1 : 0);
